...
[Next: Removing from a Binary Search
Tree](/~rhowell/DataStructures/redirect/bst-remove)       [Previous:
Binary Trees](/~rhowell/DataStructures/redirect/binary-trees)       [Up:
Binary Search
Trees](/~rhowell/DataStructures/redirect/binary-search-trees)      
...
Because 41 \< 54, we can safely ignore the right child, as all keys
there must be greater than 54. We therefore compare 41 to the key in the
root of the left child. Because 41 \> 23, we look in the right child,
and compare 41 to 35. Because 41 \> 35, we look in the right child,
where we find the key we are looking for.
...
the middle element in the tree - in fact, if one of its children is
empty, it will be either the first or the last element. In many
applications, however, when we build a binary search tree as we will
describe below, the root of the tree tends to be roughly the middle
element. When this is the case, looking up a key is very efficient.
[Later](/~rhowell/DataStructures/redirect/avl-trees), we will show how
we can build and maintain a binary search tree so that this is always
the case.
...
"10" \< "5" because '1' \< '5'). For this reason, it is usually better
to use either letters, words, or integers that all have the same number
of digits.
...
<span class="small">Last modified: Tue Jul 17 03:22:43 CDT 2018</span>
