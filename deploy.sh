...
date | tee ${LOG_FILE}
echo "pull and clean book html" | tee ${LOG_FILE}
cd ../cis300-book-html
git pull
git rm -rf .
git clean -fxd

echo "pull cis300-book and submodules" | tee ${LOG_FILE}
cd ../cis300-book
git pull --recurse-submodules

echo "running hugo to generate html" | tee ${LOG_FILE}
hugo
echo "pushing HTML to cis300-book-html" | tee ${LOG_FILE}
cd ../cis300-book-html
git add .
git commit -m "Automatic Deploy on `date +'%Y-%m-%d %H:%M:%S'`"
git push -u origin master

echo "pull on cs linux and copy to web" | tee ${LOG_FILE}
